...
const db = require("../models/MusicBot.model");
module.exports = {
  name: "skip",
    ...
    try {
      ...
      if (number) {
        if (!queue.songs.length > number)
          return interaction
            .reply({ content: lang.msg82, ephemeral: true })
            .catch((e) => {});
        if (isNaN(number))
          return interaction
        ...
        try {
          ...
          await client.player.jump(interaction, number).then((song) => {
            return interaction
              .reply({ content: `**${old.name}**, ${lang.msg83}` })
              .catch((e) => {});
          });
        } catch (e) {
          return interaction
        ...
        try {
          ...
          return interaction
            .reply({
              content: success ? `**${old.name}**, ${lang.msg83}` : lang.msg41,
            })
            .catch((e) => {});
        } catch (e) {
          return interaction
